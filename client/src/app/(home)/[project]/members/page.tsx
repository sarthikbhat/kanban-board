    ...
    <section className="flex flex-col gap-2 p-4 px-6 w-full">
      ...
      <div className="flex flex-[0.1] w-full">
        <div className="flex flex-col">
          <div className="text-2xl font-medium text-[#395886]">Test Project's Collaborators</div>
        ...
        <div
          ...
          className="flex text-sm gap-2 items-center p-3 px-4 bg-[#395886] h-fit rounded text-white cursor-pointer"
        >
      ...
      <div className="h-full flex flex-col gap-4 flex-1 w-4/5 text-sm" style={{ maxHeight: '100%', overflowX: 'scroll' }}>
        ...
        <div className=" group px-4 py-3 flex gap-4 items-center bg-[#395886] rounded text-white">
          ...
          <div className="text-sm flex group items-center font-medium p-2 text-slate-600 hover:text-red-500 hover:bg-slate-100 transition ease-in hover:rounded--md cursor-pointer"></div>
        </div>
        <div className="members-outer flex flex-col gap-2" style={{ maxHeight: '82%', overflowX: 'scroll' }}>
          {filteredUsers?.map((e, index) => {
            return (
              <div key={index} className=" group px-4 py-2 flex gap-4 items-center bg-[#B1C9EF]/50">
        ...
        {!users?.length && (
          <div className="flex justify-center items-center bg-[#D5DEEF] p-2 rounded text-[#395886] font-semibold">
            No Users
          </div>
        )}
        {!!users?.length && (
          <div className="text-slate-400 flex items-center gap-2 justify-center ml-[15%]">
            <SkipPreviousOutlinedIcon
              className={
                (currentPage - 1) * ITEMS_PER_PAGE - ITEMS_PER_PAGE < 0
                  ? 'text-slate-300/80 cursor-pointer'
                  : 'cursor-pointer'
              }
              onClick={() => navigateToPage(0, 1, -1)}
            />
            <NavigateBeforeOutlinedIcon
              ...
              onClick={() => navigateToPage((currentPage - 1) * ITEMS_PER_PAGE - ITEMS_PER_PAGE, currentPage - 1, -1)}
            />
            Showing items {getOffsetLength()} of {users?.length}
            <NavigateNextOutlinedIcon
              className={getOffsetLength() == users?.length ? 'text-slate-300/80 cursor-pointer' : 'cursor-pointer'}
              onClick={() => navigateToPage(currentPage * ITEMS_PER_PAGE, currentPage + 1, 1)}
            />
            <SkipNextOutlinedIcon
              className={getOffsetLength() == users?.length ? 'text-slate-300/80 cursor-pointer' : 'cursor-pointer'}
              onClick={() => navigateToPage(users?.length - ITEMS_PER_PAGE, Math.ceil(users?.length / ITEMS_PER_PAGE), 1)}
            />
          </div>
        )}
      </div>
