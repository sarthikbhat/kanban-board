...
import { IComment } from "./TaskModal";
...
const CommentInput: FC<InputProps> = ({
    ...
    return (
        <div data-type={type} className={"flex gap-2 w-full relative" + parentPosition}>
            <div className={"text-xs w-[5vh] p-[0.5rem] bg-white border rounded-full self-start text-center " + (type === "new" ? "mt-3" : "")}>
                {(commentData.commentedBy || JSON.parse(window.localStorage.getItem("user") || "{}")?.fullName)?.split(" ").map((e: string) => e.charAt(0)).splice(0, 2)}
            </div>
            {
                type === "comments" && !edit ?
                    <div className="p-3 border border-[#000000]/10 text-sm w-full pt-[7%] min-h[10vh] pb-[3%] group">
                        ...
                        <div className="absolute top-2 right-2 flex gap-1 text-[#000]/70">
                            <EditIcon className="text-base hidden group-hover:block hover:cursor-pointer" onClick={() => setEdit(!edit)} /> <DeleteIcon className="text-base hover:cursor-pointer hover:text-red-500 hidden group-hover:block " />
                        </div>
                        <div className="text-xs absolute top-[1vh] left-[7vh] text-[#000]/50 flex items-center gap-2"><div className="font-medium text-underline text-sm">{commentData.commentedBy}</div><div className="underline">{getUpdatedAt(commentData?.updatedAt)}</div></div>
                    </div>
                    :
                    <ReactQuill onBlur={callMe} placeholder={placeholder} className={"w-4/5 max-w-4/5 " + (extraCss ? extraCss : "")} theme="snow" value={value} onChange={(newValue: any, delta: any, source: any) => handleChangeChild(newValue, source, id)} />
            }
        </div >
    );
};
