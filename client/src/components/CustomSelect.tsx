"use client"
import { IColumns, IUser } from "@/app/addproject/page";
...
const CustomSelect: FC<IModalProps> = ({ showList = false, defaultSelectText, options, colors, isUser = true, extraCss, parentCss, setData, isState, disabled = false, displayLabel = false, extraCssForLabel, label, error, id }) => {
        ...
        <div className={"custom-select-container text-ellipsis " + (!!parentCss?.length ? parentCss : "")}>
            {displayLabel && (
                <label
                    ...
                    className={
                        ...
                        (parentCss === "items-center" ? "ml-20" : "") + (error && error[id]?.message ? " !text-red-500" : "" + " " + extraCssForLabel)
                    }
                >
                    {label}
                </label>
            ...
            >
                {isUser && <div className={"text-xs p-[0.3rem] bg-white border drop-shadow-lg rounded-full self-start" + (defaultSelectTextState === "Assign User" ? " !text-white !drop-shadow-none" : "")}>
                    {defaultSelectTextState?.toString().split(" ").map((e) => e.charAt(0)).splice(0, 2)}
                </div>}
                {(isState && defaultSelectTextState) && <span style={{ background: colors && colors[defaultSelectTextState] }} className={`rounded-full w-4 h-4`}></span>}
            ...
            {showOptionList && (
                <ul className="select-options bg-white mt-1 shadow">
                    {optionsList.map((option: any) => {
                        return (
                            <li
                                className={"custom-select-option p-2 px-5 border-b-2 flex gap-4 items-center cursor-pointer hover:bg-[#B1C9EF]/50 text-sm " + (defaultSelectText === (option.fullName || option) ? "bg-[#B1C9EF]/30" : "")}
                                data-name={option.fullName || option}
                                key={option.userName || option}
                                onClick={handleOptionClick}
                            >
                                {isState && <span style={{ background: colors && colors[option] }} className={`rounded-full w-4 h-4`}></span>}
