...
const Navbar: FC = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') setuser(JSON.parse(window.localStorage.getItem('user') || '{}'));
  ...
  return (
    <header className="flex bg-[#395886] p-3 gap-4 px-5 items-center " style={{ height: '8vh' }}>
      ...
      <h1 className="text-xs text-white font bg-[#A0A8B5]/10 hover:bg-[#A0A8B5]/30  cursor-pointer transition ease-in p-2 rounded">
        Starred Projects
      </h1>
      <h1 className="text-xs text-white font bg-[#A0A8B5]/10 hover:bg-[#A0A8B5]/30  cursor-pointer transition ease-in p-2 rounded">
        All Projects
      ...
      <Input
        ...
        displayLabel={false}
        extraCss="!p-1.5 !text-sm text-slate-500 !w-full !shadow-none"
        parentPosition="items-center !w-1/5"
      />
      ...
      <div className="flex gap-2 items-center" onClick={() => setopenPopUp(true)}>
        ...
        <div className="rounded-full p-2 text-sm border border-slate-400 shadow-lg text-slate-700 bg-white cursor-pointer">
          {user.fullName && user?.fullName.split(' ').map((e: any) => e.charAt(0))}
        </div>
        {openPopUp && (
          <div
            id="popup"
            onClick={performLogout}
            className="absolute bg-white shadow-lg rounded p-3 text-slate-800 z-[111] border-[0.1px] border-slate-300 flex gap-2 right-[0.5%] top-[6.5%] cursor-pointer"
          >
