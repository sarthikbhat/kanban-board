...
const SideMenu: FC = ({ }) => {
    ...
    return (
        <>
                ...
                <div className="flex gap-2 justify-between items-center font-medium text-white p-3 w-full">
                    <div className='flex gap-3 items-center justify-center text-lg'>
                        <div className="flex p-2 bg-[#1B3358] rounded-full w-9 h-9 items-center justify-center">
                            {projectName?.split(" ").map((e) => e.charAt(0)).splice(0, 2) + (projectName.split(" ").length === 1 ? projectName.split("").pop() || "" : "")}
                        </div>
                        <div>{projectName}</div>
                    </div>
                    <div onClick={() => clickHandle()} className='bg-[#1B3358] rounded-full w-[1.6vw] h-[1.6vw] flex items-center justify-center cursor-pointer'>
                        <ArrowBackIosNewTwoToneIcon className='text-[1rem] ' />
                    </div>
                </div>
            ...
            {styles === "closed" && <div className='opacity-100 flex flex-col bg-[#395886] border-r items-center border-t-[0.1px] w-[2vh]'>
                <div onClick={() => clickHandle("open")} className='bg-[#1B3358] shadow-lg shadow-slate-400 relative left-[50%] top-[3%] z-11 rounded-full w-[1.6vw] h-[1.6vw] flex items-center justify-center cursor-pointer'>
                    <ArrowForwardIosTwoToneIcon className='text-[1rem] text-white' />
                </div>
            </div>}
        </>
    );
